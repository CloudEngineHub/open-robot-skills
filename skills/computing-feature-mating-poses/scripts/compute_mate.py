...
def _move_loop_poses_to_shaft_interior(
    ...
    # The requested depth is enforced exactly below, so this displacement is
    # not applied twice.
    ...
    def feature_world_and_position(pose_name: str):
        ...
        feature_world = [
            float(position[key]) + rotated[index] for index, key in enumerate(("x", "y", "z"))
        ]
    ...
    for pose_name in ("approach_pose", "engaged_pose"):
        ...
        # A long, thin held body may settle in the hand after its last wrist
        # observation. The caller may cross the shaft slightly high, where the
        # loop has generous free space, and let the separate mate waypoint
    ...
    for pose_name, requested_depth in (("engaged_pose", engaged_depth), ("mate_pose", mate_depth)):
        feature_world, position = feature_world_and_position(pose_name)
        current_depth = -sum(
            (feature_world[index] - tip[index]) * outward_axis[index] for index in range(3)
        )
